...
void featureDetectionSIFT(cv::Mat image, std::vector<cv::Point2f>& points)  
{
...
void appendNewFeatures(cv::Mat& image, FeatureSet& current_features)
{
   ...
   // featureDetectionSIFT(image, points_new);
    //featureDetectionORB(image, points_new);
