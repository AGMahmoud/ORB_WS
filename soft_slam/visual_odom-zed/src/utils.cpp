...
{
	//FILE *pose_out, *pose_diff;
	//pose_out=fopen("pose_out.txt","w");
	//pose_diff=fopen("pose_diff.txt","w");
    char text[100];
    ...
    if (show_gt)
    {
      ...
      //fprintf(pose_out,"%d\t%f\t%f\t%f\n",frame_id,pose_gt.at<double>(0),pose_gt.at<double>(1),pose_gt.at<double>(2));
      //fprintf(pose_diff,"%d\t%f\t%f\t%f\n",frame_id,pose_gt.at<double>(0)-pose_matrix_gt[frame_id].val[0][3],pose_gt.at<double>(1)-pose_matrix_gt[frame_id].val[0][7],pose_gt.at<double>(2)-pose_matrix_gt[frame_id].val[0][11]);
      //printf("%d\t%f\t%f\t%f\n",frame_id,pose_gt.at<double>(0),pose_gt.at<double>(1),pose_gt.at<double>(2));
      //printf("%d\t%f\t%f\t%f\n",frame_id,pose_gt.at<double>(0)-pose_matrix_gt[frame_id].val[0][3],pose_gt.at<double>(1)-pose_matrix_gt[frame_id].val[0][7],pose_gt.at<double>(2)-pose_matrix_gt[frame_id].val[0][11]);
      cout<<"***************************************************************\n";
...
void loadImageLeft(cv::Mat& image_color, cv::Mat& image_gary, int frame_id, std::string filepath){
    char file[200];
    sprintf(file, "left/left%06d.png", frame_id);
...
void loadImageRight(cv::Mat& image_color, cv::Mat& image_gary, int frame_id, std::string filepath){
    char file[200];
    sprintf(file, "right/right%06d.png", frame_id);
    cout<<file<<endl;
    // sprintf(file, "image_0/%010d.png", frame_id);
    std::string filename = filepath + std::string(file);
