...
FILE *data_out, *timestamps;
data_out=fopen("data_out.txt","w");
timestamps=fopen("img_timestamps.txt","r");
   ...
   /* if(argc < 3)
    ...
    // Sequence
    string filepath = string("/home/seif/ssd_workspace/data/Data4/ZED/sequence/");
    ...
    float fx_l = 698.9;         //fSettings["Camera.fx"];
    float fy_l = 694.89;         //fSettings["Camera.fy"];
    float cx_l = 645.562;         //fSettings["Camera.cx"];
    float cy_l = 343.643;   
    float fx_r = 700.95;         //fSettings["Camera.fx"];
    float fy_r = 700.95;         //fSettings["Camera.fy"];
    float cx_r = 654.986;         //fSettings["Camera.cx"];
    float cy_r = 348.044;         //fSettings["Camera.cy"];
          //fSettings["Camera.cy"];
    float bf = -63.0377 ;       //fSettings["Camera.bf"];

    cv::Mat projMatrl = (cv::Mat_<float>(3, 4) << fx_l, 0., cx_l, 0., 0., fy_l, cy_l, 0., 0,  0., 1., 0.);
    cv::Mat projMatrr = (cv::Mat_<float>(3, 4) << fx_r, 0., cx_r, bf, 0., fy_r, cy_r, 0., 0,  0., 1., 0.);
...
fprintf(data_out,"%d\n",init_frame_id);
    ...
    for (int frame_id = init_frame_id+1; frame_id < 5075; frame_id++)
    {
        ...
        fscanf(timestamps,"%d\t%llu\t%llu\n",&x,&ts,&tmp);
fprintf(data_out,"%d\t%llu\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",frame_id,ts,-1*translation_stereo.at<double>(0,0),translation_stereo.at<double>(0,1),-1*translation_stereo.at<double>(0,2),-1*pose.at<double>(0,0),pose.at<double>(0,1),-1*pose.at<double>(0,2),-1*Rpose_euler(0),Rpose_euler(1),-1*Rpose_euler(2));
        clock_t toc = clock();
